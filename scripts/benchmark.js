...
class Benchmark {
  ...
  async runAllBenchmarks() {
    ...
    await this.runBenchmark('Small Diff Processing', async () => {
      ...
      processor.processDiff(smallDiff, 'test.js');
    }, 500);

    // Benchmark 2: Large diff processing
    await this.runBenchmark('Large Diff Processing', async () => {
      processor.processDiff(largeDiff, 'large-file.js');
    }, 100);
    ...
    await this.runBenchmark('Multiple Files Processing', async () => {
      ...
      files.forEach(file => {
        processor.processDiff(file.patch, file.filename);
      });
    }, 200);
    ...
    await this.runBenchmark('VersionSafeUtils Array Generation', async () => {
      utils.generateArray(100, 'test');
    }, 1000);

    await this.runBenchmark('VersionSafeUtils String Operations', async () => {
      utils.stringIncludes('test string', 'string');
    }, 5000);
